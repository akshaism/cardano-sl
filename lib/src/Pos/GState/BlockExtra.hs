...
import           Pos.DB (DBError (..), MonadDB, MonadDBRead (..), RocksBatchOp (..),
                         dbSerializeValue)
import           Pos.DB.GState.Common (MonadBlockDBSafe, getHeader, gsGetBi, gsPutBi)
...
loadHeadersUpWhile
    :: (MonadBlockDBSafe m, HasHeaderHash a)
    => a
...
loadBlocksUpWhile
    :: (MonadDBRead m, HasHeaderHash a)
    => a
    -> (Block -> Int -> Bool)
    -> m (OldestFirst [] Block)
loadBlocksUpWhile start condition = loadUpWhile dbGetBlock identity start condition
